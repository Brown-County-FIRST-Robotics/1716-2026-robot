...
public class RobotContainer {
  ...
  private void configureButtonBindings() {
    ...
    controller
        ...
        .whileTrue(
            Commands.run(
                () -> shooter.quickServoCommand(SmartDashboard.getNumber("Hood Servo", 2)),
                shooter));
    ...
    controller
        .rightTrigger(0.7)
        .whileTrue(
            Commands.run(() -> shooter.quickWheelCommand(8), shooter)
                .alongWith(
                    Commands.waitSeconds(1.0)
                        .andThen(Commands.run(() -> rollers.setSpeeds(0, 30), rollers)))
                .alongWith(
                    Commands.race(
