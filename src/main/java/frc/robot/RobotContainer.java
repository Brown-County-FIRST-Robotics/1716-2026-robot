...
public class RobotContainer extends PeriodicRunnable {
  ...
  private void configureButtonBindings() {
    ...
    // Dpad
    controller.povUp().onTrue(Commands.runOnce(() -> drive.targetAngle = Rotation2d.kZero));
    controller.povRight().onTrue(Commands.runOnce(() -> drive.targetAngle = Rotation2d.kCW_90deg));
    controller.povDown().onTrue(Commands.runOnce(() -> drive.targetAngle = Rotation2d.k180deg));
    controller.povLeft().onTrue(Commands.runOnce(() -> drive.targetAngle = Rotation2d.kCCW_90deg));
    shooter.register();
  }
