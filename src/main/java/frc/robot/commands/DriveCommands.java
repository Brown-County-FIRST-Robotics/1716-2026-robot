...
public class DriveCommands {
  ...
  private static final double DISTANCE_KD = 0.1;
  ...
  public static Command joystickDrive(
    ...
    AngleHoldState hold = new AngleHoldState(drive.getRotation());
    return Commands.run(
            () -> {
              ...
              if (Math.abs(omega) < DEADBAND) {
                if (!hold.holding) {
                  hold.holding = true;
                  hold.target = drive.getRotation();
                  angleController.reset(drive.getRotation().getRadians());
                  omega = 0;
                } else
                  omega =
                      angleController.calculate(
                          drive.getRotation().getRadians(), hold.target.getRadians());
              } else {
                hold.holding = false;
